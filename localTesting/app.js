...
(function (Pairings) {
    let Testing;
    (function (Testing) {
        function comparePreformance(input, runs = 1000) {
            input = [...input];
            if (input.length % 2 !== 0)
                input.push(null);
            return {
        ...
        Testing.comparePreformance = comparePreformance;
        function doTest(players, func, runs) {
            let costs = [];
            let start = Date.now();
    ...
    })(Testing = Pairings.Testing || (Pairings.Testing = {}));
    ...
    function whiteFraction(history) {
     ...
     * @returns
     */
    function getCosts(player, others) {
        let output = [];
    ...
    const _colors = function (pairing) { return Object.keys(pairing); }({ black: null, white: null });
    function duplicatePairing(pairing) {
        return { white: pairing.white, black: pairing.black };
    }
    function getBestNeighboor(input) {
        let best = input;
     ...
     * @param input
     */
    function swapRandomPlayers(input) {
        let length = input.length;
...
let club = Benji.objToArray_dropKey(JSON.parse(text));
